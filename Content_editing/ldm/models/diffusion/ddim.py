...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        self.vgg_content_filter = VGG16Encoder(need_ref=True).cuda()
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            if mask is not None:
                ...
                img = img * mask + (1. - mask) * tt

            outs = self.p_sample_ddim_magic_conditional_v2(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
            ...
            for x_sample in pred_x0_torch:
                ...
                img_save.save(os.path.join("/home/hexiangyu/FreeDoM/Content_editing/intermediates", "{}_{}.png".format(count1, count2)))
    ...
    # @torch.no_grad()
    def p_sample_ddim_conditional2(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,i=0):
        ...
        for j in range(repeat):
            ...
            if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                ...
                x_in = torch.cat([x] * 2)
                t_in = torch.cat([t] * 2)
                c_in = torch.cat([unconditional_conditioning[i].expand(*c.shape).to(device), c])
            ...
            if start > index >= end:
                ...
                content_grad = torch.autograd.grad(outputs=content_loss, inputs=x)[0]
                
                norm_grad = content_grad
