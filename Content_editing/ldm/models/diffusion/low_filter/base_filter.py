...
class VGG16ContentEncoder(nn.Module):
    def __init__(
        self, need_ref=False, ref_path=None, ref_mask_path=None, image_size=256
    ):
        super().__init__()
        ...
        if need_ref:
            ...
            print('[Debug] Content ref_path : ', ref_path)
            print('[Debug] Content ref_mask_path : ', ref_mask_path)

            content = load_image(self.ref_path, size=(self.image_size, self.image_size))
            self.ref = style_transform(content).repeat(1, 1, 1, 1).cuda()
            ...
            if self.ref_mask_path is not None:
                ...
                self.mask_tensor = F.interpolate(
                    torch_array, size=content.size, mode="nearest"
                )
...
class VGG16StyleEncoder(nn.Module):
    def __init__(
        ...
        if need_ref:
            ...
            print('[Debug] Style ref_mask_path : ', ref_mask_path, flush=True)
